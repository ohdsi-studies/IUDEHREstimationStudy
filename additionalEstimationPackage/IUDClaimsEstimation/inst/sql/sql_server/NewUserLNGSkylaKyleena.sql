...
(
  select concept_id from @vocabulary_database_schema.CONCEPT where concept_id in (46221626,19131647,19132304)
...
(
  select concept_id from @vocabulary_database_schema.CONCEPT where concept_id in (43012751,1718412)
...
(
  select de.*
  FROM @cdm_database_schema.DRUG_EXPOSURE de
JOIN #Codesets codesets on ((de.drug_concept_id = codesets.concept_id and codesets.codeset_id = 49))
) C
